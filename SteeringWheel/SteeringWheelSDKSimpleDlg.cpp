...
{
	//inizializzazione
	CSteeringWheelSimpleDlg::OnBnClickedLogiSteeringInitialize();
	...
	//Bind the socket to an ip adress and port to a socket

	std::cout << "ciao" << std::endl;
	sockaddr_in hint;
	hint.sin_family = AF_INET;
	hint.sin_port = htons(4000);
	inet_pton(AF_INET, "192.168.0.2", &hint.sin_addr.s_addr);
	//bind(listening, (sockaddr*)&hint, sizeof(hint));

	//Tell Winsock the socket is for listening

	//listen(listening, SOMAXCONN); //where SOMAXCONN is the number of maximum connections we can have as a backlog 
	...
	if (clientSocket == INVALID_SOCKET) {
		::SetWindowText(::GetDlgItem(m_hWnd, IDC_RESULT), L"The socket is invalid");
		return;
	}*/
	...
	while (flag == 0) {
		//::SetWindowText(::GetDlgItem(m_hWnd, IDC_RESULT), L"Socket started!");
		ZeroMemory(buf, 4096);
		...
		if (buf[0] == 'i')
		{
			::SetWindowText(::GetDlgItem(m_hWnd, IDC_RESULT), L"Vibration");
			CSteeringWheelSimpleDlg::OnBnClickedLogiPlayBumpyRoadEffect();
		...
		{
			::SetWindowText(::GetDlgItem(m_hWnd, IDC_RESULT), L"Stop vibration");
			CSteeringWheelSimpleDlg::OnBnClickedLogiStopBumpyRoadEffect();
